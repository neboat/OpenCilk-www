...
class CilkBookFormatter(Formatter):
    ...
    def __init__(self, **options):
        ...
        # Options added with cilkhilite pygments plugin
        if 'verbenvironment' in options.keys():
            self.envname = options.get('verbenvironment')
    ...
    def format_unencoded(self, tokensource, outfile):
        ...
        if not self.inline:
            ...
            if self.saveverbatimname != "":
                outfile.write(u'{' + self.saveverbatimname + u'}')
            outfile.write(u'\n')
        ...
        for ttype, value in tokensource:
            ...
            elif ttype not in Token.Escape and ttype not in Token.Comment.PureTeX:
                value = escape_tex(value, cp)
            styles = []
