...
from pygments.lexer import Lexer, RegexLexer, DelegatingLexer, \
    include, bygroups, using, words, this, combined, inherit
...
class CilkLexer(CppLexer):
    ...
    tokens = {
        ...
        'template': [
            ...
            (r'(\d+[LlUu]*)', Number.Integer, ('#pop', 'template-args')),
...
class GasCBLexer(GasLexer):
    ...
    tokens = {
        'root': [
            ...
            (r'(j|call|fcom|fucom)' + identifier,
             Name.Function, 'instruction-args-source'),
            (r'(cmp|test)' + identifier,
             Name.Function, 'instruction-args-source'),
...
class LlvmCBLexer(RegexLexer):
    """
    For LLVM assembly code.
    """
    name = 'LLVM'
    aliases = ['llvmcb', 'tapir']
    filenames = ['*.ll']
    mimetypes = ['text/x-llvm']

    #: optional Comment or Whitespace
    string = r'"[^"]*?"'
    identifier_np = r'[-a-zA-Z$._][\w\-$.]*|' + string
    # identifier = r'([-a-zA-Z$._][\w\-$.]*|' + string + ')'
    identifier = r'(' + identifier_np + ')'
    ...
    tokens = {
        'root': [
            ...
            (r'(declare|define)(\s+)([[a-zA-Z_]+\s+]*)(' + identifier_np +
                 ')(\s+)(@' + identifier_np + ')([(])',
            bygroups(Keyword, Text, using(this), using(this), Text, Name.Function,
                         Punctuation)),

            (r'(declare|define)(\s+)(' + identifier_np +
                 ')(\s+)(@' + identifier_np + ')([(])',
            bygroups(Keyword, Text, using(this), Text, Name.Function,
                         Punctuation)),
            ...
            (r'0[xX][a-fA-F0-9]+', Number.Integer),
            (r'-?\d+(?:[.]\d+)?(?:[eE][-+]?\d+(?:[.]\d+)?)?', Number),
        ...
        'whitespace': [
            (r'(\n|\s)+', Text),
            (r'(;;)(.*?)(\n)', bygroups(Comment.Invisible, Comment.PureTeX, Text)),
            (r'(;.*?)(;;)(.*?)(\n)', bygroups(Comment, Comment.Invisible, Comment.PureTeX, Text)),
            (r';.*?\n', Comment)
        ],
        'type-keywords': [
            # Types
        ...
        'keyword': [
            ...
            # Constants; colored like builtins, in order to imitate Emacs llvm-mode.
            (words((
                'alwaysinline', 'argmemonly', 'builtin', 'cold', 'convergent', 'dso_local',
                'inaccessiblememonly', 'inaccessiblemem_or_argmemonly', 'inlinehint',
                'jumptable', 'local_unnamed_addr', 'minsize', 'naked', 'nobuiltin',
                'noalias', 'nocapture', 'noduplicate', 'noimplicitfloat', 'noinline', 'nonlazybind',
                'nonnull', 'noredzone', 'noreturn', 'norecurse', 'nounwind', 'optnone',
                'optsize', 'readnone', 'readonly', 'returns_twice', 'speculatable', 'ssp',
                'sspreq', 'sspstrong', 'safestack', 'sanitize_address', 'sanitize_cilk',
                'sanitize_hwaddress', 'sanitize_thread', 'sanitize_memory', 'stealable',
                'strictfp', 'unnamed_addr', 'uwtable', 'writeonly'),
            ...
            # Tapir keywords
            (words(('detach', 'reattach', 'sync', 'within'), suffix=r'\b'), Keyword.Cilk)
        ]
    }
